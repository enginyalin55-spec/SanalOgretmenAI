...
// --- TDK KURAL SÖZLÜĞÜ (Frontend için Açıklamalar) ---
const TDK_LOOKUP = {
...
export default function MainScreen({ user, setUser }) {
  ...
  const resetFlow = () => { 
    setStep(1); setImage(null); setOcrText(""); setResult(null); setImageUrl(""); setActiveErrorData(null); 
  };

  const takePhoto = async () => {
    const p = await ImagePicker.requestCameraPermissionsAsync(); 
    if (!p.granted) return showAlert("İzin", "Kamera izni gerekli.");
    const r = await ImagePicker.launchCameraAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, allowsEditing: true, quality: 0.7, base64: true });
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.badgeText}>{classCode} • {studentLevel}</Text>
          </View>
        ...
        <View style={styles.tabsContainer}>
          <TouchableOpacity style={[styles.tab, activeTab==='new'&&styles.activeTab]} onPress={()=>setActiveTab('new')}>
            <Text style={styles.tabText}>📝 Yeni Ödev</Text>
          </TouchableOpacity>
              ...
              {step === 1 && (
                <>
                  ...
                  <TouchableOpacity style={[styles.sendButton, {opacity:image?1:0.5}]} onPress={startOCR} disabled={!image||loading}>
                    {loading?<ActivityIndicator color="white"/>:<Text style={styles.sendButtonText}>Tara</Text>}
                  </TouchableOpacity>
                </>
              )}
              ...
              {step === 2 && (
                <View style={{width:'100%'}}>
                  ...
                  <View style={styles.ocrPreviewCard}>
                    <OcrUncertaintyText text={ocrText} onPressHint={(span)=>setActiveOcrHintData({span, x:SCREEN_WIDTH/2, y:SCREEN_HEIGHT/2})} />
                  </View>
            ...
            {step === 3 && result && (
              <View style={styles.resultContainer}>
                ...
                <TouchableOpacity onPress={resetFlow} style={[styles.sendButton, {backgroundColor:'#34495e', marginTop:20}]}>
                  <Text style={styles.sendButtonText}>Yeni Yükle</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
          ...
          <View style={styles.contentArea}>
            <FlatList data={historyData} keyExtractor={item=>item.id.toString()} renderItem={({item}) => (
              <View style={styles.historyCard}>
                ...
                <TouchableOpacity onPress={()=>{setSelectedHistoryItem(item); setShowDetailOverlay(true);}} style={{backgroundColor:'#3498db', padding:10, borderRadius:8, marginTop:5}}>
                  <Text style={{color:'white'}}>Rapor</Text>
                </TouchableOpacity>
              </View>
            )} />
          </View>
        ...
        <View style={styles.fullScreenOverlay}>
          <View style={styles.detailContainer}>
            <View style={styles.sheetHeader}>
              ...
              <TouchableOpacity onPress={()=>setShowDetailOverlay(false)}><Text style={styles.closeBtnText}>✕</Text></TouchableOpacity>
            </View>
            <ScrollView contentContainerStyle={{padding:20}}>
              ...
              {historyErrors.map((err, index) => (
                <TouchableOpacity key={index} style={styles.errorItem} onPress={()=>setActiveErrorData({err, x:SCREEN_WIDTH/2, y:SCREEN_HEIGHT/2})}>
                  <Text style={styles.errorText}>
                    <Text style={{textDecorationLine:'line-through', color:'#e74c3c'}}>{err.wrong}</Text> ➜ {err.correct}
                  </Text>
                  <Text style={styles.errorDesc}>{err.explanation}</Text>
                </TouchableOpacity>
              ))}
              {selectedHistoryItem.analysis_json?.teacher_note && (
                <View style={styles.noteCard}>
                  ...
                  <Text>{selectedHistoryItem.analysis_json.teacher_note}</Text>
                </View>
              )}
            </ScrollView>
          </View>
