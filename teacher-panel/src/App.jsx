// App.jsx  ✅ (ESKİ ARAYÜZ + YENİ TDK/CEFR İŞLEVSİLLİĞİ + ESKİ PDF PORTRAIT)
// Not: Bu dosya, senin “eski App.jsx görünümü”nü KORUR.
// Yeni sistemde gelen errors (span + rule_id) varsa onları kullanır; yoksa eski wrong/correct aramasıyla da çalışır.

import { useEffect, useState } from 'react';
import { supabase } from './supabase';
import {
  BarChart2, Save, Edit3, Globe, Download, LogOut, Lock,
  Plus, Trash2, CheckCircle, Maximize2, X, ZoomIn, ZoomOut
} from 'lucide-react';
import {
...
// (Eski kodunda vardı) Backend adresi — puan kaydetme endpoint’i varsa kullanır, yoksa Supabase fallback yapar.
const API_URL = "https://sanalogretmenai.onrender.com";

// --- TDK KURAL SÖZLÜĞÜ (YENİ İŞLEVSİLLİK) ---
const TDK_LOOKUP = {
...
// --- YENİ/ESKİ ERROR UYUMLULUK ADAPTER ---
function ruleTitle(err) {
  const rid = err?.rule_id || err?.ruleId || err?.rule || err?.tdk_rule_id;
  if (!rid) return null;
  return TDK_LOOKUP[rid] || rid;
}

// Hata listesi farklı backend’lerden gelebilir:
// - Yeni: { span:{start,end}, wrong, correct, explanation, rule_id, type }
// - Eski: { wrong, correct, explanation, type } (span yok)
function normalizeErrors(errors, text) {
  if (!Array.isArray(errors)) return [];
  return errors
    .map((e) => {
      const span = e?.span && typeof e.span.start === 'number' && typeof e.span.end === 'number'
        ? { start: e.span.start, end: e.span.end }
        : null;

      const wrong = e?.wrong ?? e?.incorrect ?? e?.error ?? "";
      const correct = e?.correct ?? e?.suggestion ?? e?.fix ?? "";
      const explanation = e?.explanation ?? e?.message ?? e?.detail ?? "";
      const type = e?.type ?? "";
      const rid = e?.rule_id ?? e?.ruleId ?? e?.rule ?? e?.tdk_rule_id ?? "";

      // Eğer span yok ama wrong var ise, eski arama yöntemi çalışsın.
      // Eğer span var ama wrong boşsa, span’dan wrong’u metinden çıkarabiliriz.
      let finalWrong = (wrong || "").toString();
      if (!finalWrong && span && text) {
        try { finalWrong = text.slice(span.start, span.end); } catch { /* ignore */ }
      }
      ...
      return {
        ...
        type: (type || "").toString(),
        rule_id: (rid || "").toString()
      };
    })
    .filter(e => (e.wrong && e.wrong.trim().length > 0) || e.span);
}
...
// ✅ Yeni sistem: span (start/end) ile boyar.
// ✅ Eski sistem: wrong metnini arayıp boyar.
// Tooltip içine TDK kural adını da ekler.
const HighlightedText = ({ text, errors }) => {
  if (!text) return null;

  const safeErrors = normalizeErrors(errors, text);
  if (safeErrors.length === 0) return <div style={{ whiteSpace: 'pre-wrap' }}>{text}</div>;

  // 1) Span varsa: span’lı aralıkları topla
  const spanRanges = [];
  safeErrors.forEach(err => {
    if (err.span && typeof err.span.start === 'number' && typeof err.span.end === 'number') {
      const s = Math.max(0, err.span.start);
      const e = Math.min(text.length, err.span.end);
      if (s < e) {
        spanRanges.push({ start: s, end: e, error: err });
      }
    }
  });

  // 2) Span yoksa: eski yöntemle wrong araması
  const ranges = [];
  if (spanRanges.length > 0) {
    ranges.push(...spanRanges);
  } else {
  ...
  uniqueRanges.forEach((range, i) => {
    ...
    const tdkTitle = ruleTitle(range.error);
    const tooltipLines = [
      `❌ Yanlış: ${range.error.wrong || "-"}`,
      `✅ Doğrusu: ${range.error.correct || "-"}`,
      tdkTitle ? `📌 TDK: ${tdkTitle}` : null,
      range.error.explanation ? `💡 Açıklama: ${range.error.explanation}` : null
    ].filter(Boolean).join("\n");
...
export default function App() {
  ...
  useEffect(() => {
    const initSession = async () => {
      // İstersen bunu kapatabilirsin; eski kodunda vardı.
  ...
  useEffect(() => {
    if (selectedSubmission) {
      ...
      setCalculatedTotal(selectedSubmission.score_total || 0);
  ...
  // ✅ Puanı kaydet: önce API endpoint’i dener; olmazsa supabase update fallback
  async function saveUpdatedScore() {
    ...
    try {
      ...
      // API çalışmıyorsa supabase fallback'a geçeceğiz
    }

    // 2) Supabase fallback
    try {
      const { error } = await supabase.from('submissions').update({
        score_total: calculatedTotal,
      ...
      if (error) {
        ...
        setSubmissions(updatedSubmissions);
        setSelectedSubmission({ ...selectedSubmission, score_total: calculatedTotal, analysis_json: fullJson });
        alert("✅ Puan başarıyla güncellendi! (Supabase)");
        setIsScoreChanged(false);
      }
    } catch (error) {
  ...
  // --- FINAL + DÜZELTİLMİŞ: MOBİLDE YZ GİZLEME GARANTİLİ PDF (ESKİ PORTRAIT) ---
  const downloadPDF = async () => {
    ...
    style.innerHTML = `
      ...
      /* 2. Sütun: Metin ve Öğretmen Notu */
    ...
    ignored.forEach(el => {
      ...
      if (t.includes("yapay zeka") || t.includes("yapay zeka analizi")) {
    ...
    // Ek güvenlik: “Yapay Zeka Analizi” kutusu kaçarsa yok et
  ...
  function calculateStats(data) {
    ...
    data.forEach(sub => {
      const errs = normalizeErrors(sub.analysis_json?.errors, sub.ocr_text || "");
      if (errs.length) {
        errs.forEach(err => {
          const typeText = (err.type || "").toLowerCase();
          const descText = (err.explanation || "").toLowerCase();
          const ruleText = (ruleTitle(err) || "").toLowerCase();
          const fullText = `${typeText} ${descText} ${ruleText}`;
          ...
          if (fullText.includes('söz') || fullText.includes('cümle') || fullText.includes('yapı') ||
            ...
            fullText.includes('kesme') || fullText.includes('virgül') || fullText.includes('noktalama')) {
            stats['Yazım/Nokt.']++;
          }
          else if (fullText.includes('keli') || fullText.includes('sözcük') || fullText.includes('anlam') ||
            fullText.includes('seçim') || fullText.includes('ifade') || fullText.includes('kelime')) {
            stats['Kelime']++;
          }
          else {
            stats['Dilbilgisi']++;
  ...
  if (!session) {
    return (
      <div style={{
        ...
        <div style={{
          backgroundColor: 'white', padding: 40, borderRadius: 15,
          boxShadow: '0 10px 25px rgba(0,0,0,0.05)', width: 350, textAlign: 'center'
        }}>
          <div style={{
            backgroundColor: '#e8f0fe', width: 60, height: 60, borderRadius: '50%',
          ...
          <form onSubmit={handleLogin}>
            <input
              ...
              style={{ width: '100%', padding: 12, marginBottom: 15, borderRadius: 8, border: '1px solid #ddd', boxSizing: 'border-box' }}
              required
            />
            <input
              type="password"
              ...
              style={{ width: '100%', padding: 12, marginBottom: 25, borderRadius: 8, border: '1px solid #ddd', boxSizing: 'border-box' }}
              required
            />
            <button
              type="submit"
              disabled={loading}
              style={{
                width: '100%', padding: 12, backgroundColor: '#3498db', color: 'white',
    ...
    return (
      <div style={{ padding: '30px', fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", backgroundColor: '#f4f6f8', minHeight: '100vh' }}>
        ...
        <div style={{ marginBottom: 25, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h1 style={{ color: '#2c3e50', margin: 0, fontSize: 26, fontWeight: '700' }}>🎓 Öğretmen Kontrol Paneli</h1>
        ...
        <div style={{
          backgroundColor: 'white', padding: 15, borderRadius: 12, marginBottom: 25,
          boxShadow: '0 2px 10px rgba(0,0,0,0.03)', display: 'flex',
          ...
          <select
            ...
            style={{
              ...
              fontFamily: 'inherit', fontSize: 14, minWidth: 200
            }}
          >
          ...
          {isEditingClass && (
            <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ backgroundColor: '#27ae60', color: 'white', border: 'none', padding: '6px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 12 }}
              >
                Kaydet
              </button>
              <button
                onClick={() => setIsEditingClass(false)}
                style={{ backgroundColor: '#95a5a6', color: 'white', border: 'none', padding: '6px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 12 }}
              >
                İptal
              </button>
            </div>
          ...
          {showCreateClass && (
            <div style={{
              ...
              backgroundColor: '#f9f9f9', padding: 5, borderRadius: 6,
              border: '1px solid #eee'
            }}>
              <input
                type="text"
              ...
              <button
                ...
                style={{ backgroundColor: '#3498db', color: 'white', border: 'none', padding: '6px 12px', borderRadius: 4, cursor: 'pointer', fontSize: 13 }}
              >
                Kaydet
              </button>
            </div>
        ...
        <div style={{ display: 'flex', gap: 25, marginBottom: 25, flexDirection: window.innerWidth < 900 ? 'column' : 'row' }}>
          <div style={{ flex: 2, backgroundColor: 'white', padding: 25, borderRadius: 12, boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
          ...
          <div style={{ flex: 1, backgroundColor: 'white', padding: 25, borderRadius: 12, boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
            <h3 style={{ marginTop: 0, marginBottom: 20, color: '#444', display: 'flex', alignItems: 'center', gap: 10, fontSize: 16 }}>
              ...
              <tbody>
                {filteredSubmissions.map((sub) => (
                  <tr key={sub.id} style={{ borderBottom: '1px solid #f9f9f9' }}>
                    <td style={{ padding: '15px 25px', color: '#2c3e50', fontSize: 14 }}>
                    ...
                    <td style={{ padding: '15px 25px' }}>
                      {sub.classroom_code
                        ? <span style={{ backgroundColor: '#f1f2f6', padding: '3px 8px', borderRadius: 4, fontSize: 12, fontWeight: 'bold', color: '#34495e' }}>{sub.classroom_code}</span>
                    ...
                    <td style={{ padding: '15px 25px', fontSize: 14 }}>
                      {(sub.score_total || 0) >= PASS_THRESHOLD ? '✅ Geçti' : '⚠️ Tekrar'}
                    </td>
                    ...
                    <td style={{ padding: '15px 25px' }}>
                      <button
                        onClick={(e) => { e.stopPropagation(); deleteSubmission(sub.id, sub.student_name); }}
  ...
  const normalizedErrors = normalizeErrors(selectedSubmission.analysis_json?.errors, selectedSubmission.ocr_text || "");
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 15, color: '#7f8c8d', fontSize: 14 }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                🌍 {COUNTRY_NAMES[selectedSubmission.country] || selectedSubmission.country}
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                🗣️ {selectedSubmission.native_language}
              </span>
            </div>
          </div>
          ...
          <div style={{ flex: 1, width: '100%' }} className="avoid-break">
            <div style={{ backgroundColor: 'white', padding: 30, borderRadius: 12, boxShadow: '0 4px 15px rgba(0,0,0,0.05)', marginBottom: 20, breakInside: 'avoid' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <span data-html2canvas-ignore="true" style={{ backgroundColor: '#f1f2f6', padding: '5px 10px', borderRadius: 5, fontSize: 11, color: '#7f8c8d', fontWeight: 'bold' }}>
                  OCR TARAMASI
                </span>
              </div>
              ...
              <div style={{
                ...
                <HighlightedText text={selectedSubmission.ocr_text} errors={normalizedErrors} />
              </div>
            </div>

            {/* YZ KUTUSU (PDF’te gizlenir - eski davranış) */}
            <div data-html2canvas-ignore="true" style={{ backgroundColor: 'white', padding: 25, borderRadius: 12, boxShadow: '0 4px 15px rgba(0,0,0,0.05)', marginBottom: 20, borderLeft: '5px solid #ffc107', breakInside: 'avoid' }}>
              <strong style={{ color: '#d35400', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, display: 'block', marginBottom: 8 }}>
                🤖 Yapay Zeka Analizi
              </strong>
              <span style={{ color: '#5e4a18', fontSize: 15 }}>
                {selectedSubmission.analysis_json?.teacher_note || selectedSubmission.analysis_json?.ai_note || selectedSubmission.analysis_json?.insight || "-"}
              </span>
            </div>

            {/* Öğretmen Notu (PDF’te kalır) */}
            <div style={{ backgroundColor: 'white', padding: 25, borderRadius: 12, boxShadow: '0 4px 15px rgba(0,0,0,0.05)', borderLeft: '5px solid #3498db', breakInside: 'avoid' }}>
              ...
              <div data-html2canvas-ignore="true" style={{ textAlign: 'right', marginTop: 10 }}>
                <button
          ...
          <div style={{ flex: 1, width: '100%' }} className="avoid-break">
            <div style={{ backgroundColor: 'white', padding: 25, borderRadius: 12, marginBottom: 20, textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', breakInside: 'avoid' }}>
              ...
              <div style={{
                fontSize: 64,
                fontWeight: '800',
                color: calculatedTotal >= PASS_THRESHOLD ? '#27ae60' : '#e74c3c',
                margin: '5px 0'
              }}>
                {calculatedTotal}
              </div>
            ...
            <div style={{ backgroundColor: 'white', padding: 25, borderRadius: 12, boxShadow: '0 4px 15px rgba(0,0,0,0.05)', breakInside: 'avoid' }}>
              ...
              {normalizedErrors.length === 0 && <p style={{ color: '#27ae60', textAlign: 'center' }}>Hata bulunamadı. 🎉</p>}

              {normalizedErrors.map((err, i) => {
                const tdk = ruleTitle(err);
                return (
                  <div key={i} style={{ marginBottom: 15, borderBottom: '1px solid #f9f9f9', paddingBottom: 15, breakInside: 'avoid', pageBreakInside: 'avoid' }}>
                    ...
                    {tdk && (
                      <div style={{ fontSize: 12, color: '#2980b9', marginBottom: 4 }}>
                        📌 <strong>TDK:</strong> {tdk}
                      </div>
                    )}

                    <div style={{ fontSize: 13, color: '#636e72', lineHeight: 1.4 }}>
                      {err.explanation || "-"}
                    </div>
                  </div>
                );
